...
function ShopContent() {
  ...
  const [sliderValue, setSliderValue] = useState(filters.priceRange.max); // Local state for smooth sliding
  ...
  // Debounce price filter to prevent lagging
  useEffect(() => {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return result;
  }, [allProductsList, categoryParam, maxPrice, selectedColors, selectedSizes, sortBy]);
  ...
  const FilterSections = ({ isMobile = false }) => (
    <div className="space-y-10">
          ...
          {filters.categories.map((cat) => (
            <Link 
              ...
              className={`flex items-center justify-between py-1 transition-all ${
                categoryParam?.toLowerCase() === cat.toLowerCase() ? "font-bold text-black" : "text-black/50 hover:text-black"
              }`}
            >
              <span>{cat}</span>
            </Link>
      ...
      {/* Price Filter - Optimized for Performance */}
      <div className="pb-8 border-b border-black/5">
          ...
          {filters.colors.map((color) => {
             ...
             return (
              <button key={color} onClick={() => toggleColor(color)} className="group relative">
                <div 
                  className={`w-7 h-7 rounded-full border border-black/5 flex items-center justify-center transition-all ${
                    selectedColors.includes(color) ? "ring-2 ring-black ring-offset-2 scale-110" : "hover:scale-110"
                  }`}
          ...
          {filters.sizes.map((size) => (
            <button
              ...
              className={`px-5 py-2 rounded-lg text-[10px] font-bold uppercase transition-all border ${
                selectedSizes.includes(size) ? "bg-black text-white border-black" : "bg-transparent text-black/40 border-black/10 hover:border-black hover:text-black"
              }`}
            >
              {size}
            </button>
      ...
      <button onClick={resetFilters} className="w-full text-black/40 hover:text-black py-4 font-bold text-[9px] uppercase tracking-[0.2em] transition-all">
        Clear All
      </button>
    </div>
  );
  ...
  return (
    <div className="bg-white min-h-screen">
      <div className="container mx-auto px-4 md:px-16 py-8 md:py-16">
          ...
          <div className="flex items-center justify-between md:justify-end gap-8 border-t border-black/5 md:border-none pt-8 md:pt-0">
             <div className="flex items-center gap-3">
                ...
                <select 
                  ...
                  className="font-bold text-sm bg-transparent focus:outline-none cursor-pointer uppercase"
                >
             ...
             <button 
                ...
                className="lg:hidden flex items-center gap-2 px-6 py-3 bg-black text-white rounded-full text-[10px] font-bold uppercase tracking-widest active:scale-95 transition-transform"
              >
        ...
        <div className="flex flex-col lg:grid lg:grid-cols-12 gap-16 md:gap-16">
          {/* Sidebar - Desktop */}
          <aside className="hidden lg:block lg:col-span-3 h-fit sticky top-32">
             <FilterSections />
          </aside>

          {/* Product Grid - Fixed Height to prevent jumping */}
          <main className="lg:col-span-9 min-h-[800px]">
            {paginatedProducts.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center py-40 border border-dashed border-black/10 rounded-3xl">
                ...
                <button onClick={resetFilters} className="px-8 py-3 bg-black text-white rounded-full text-[10px] font-bold uppercase tracking-widest">
                   Clear Filters
                </button>
              </div>
            )}
              ...
              <div className="mt-20 flex items-center justify-center gap-12 border-t border-black/5 pt-12">
                <button 
                  ...
                  className={`text-[10px] font-bold uppercase tracking-widest transition-all ${
                    currentPage === 1 ? "opacity-10" : "hover:text-black/60"
                  }`}
                >
                  Prev
                </button>
                  ...
                  {[...Array(totalPages)].map((_, i) => (
                    <button
                      ...
                      className={`text-sm font-bold transition-all ${
                        currentPage === i + 1 ? "text-black border-b-2 border-black" : "text-black/20 hover:text-black"
                      }`}
                    >
                      {i + 1}
                    </button>
                ...
                <button 
                  ...
                  className={`text-[10px] font-bold uppercase tracking-widest transition-all ${
                    currentPage === totalPages ? "opacity-10" : "hover:text-black/60"
                  }`}
                >
                  Next
                </button>
          ...
          <>
            ...
            <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", damping: 30, stiffness: 300 }} className="fixed right-0 top-0 bottom-0 w-[85vw] max-w-sm bg-white z-[110] lg:hidden flex flex-col">
              <div className="p-6 flex items-center justify-between border-b border-black/5">
                 ...
                 <button onClick={() => setShowFilters(false)} className="p-2 hover:bg-black/5 rounded-full">
                    <X className="w-6 h-6" />
                 </button>
              </div>
              <div className="flex-1 overflow-y-auto p-6 scrollbar-hide">
                 <FilterSections isMobile={true} />
              </div>
              <div className="p-6">
                 <button onClick={() => setShowFilters(false)} className="w-full bg-black text-white py-4 rounded-xl font-bold text-[10px] uppercase tracking-widest shadow-xl">
                   Apply
                 </button>
              </div>
            </motion.div>
...
export default function ShopPage() {
  return (
    <Suspense fallback={<div className="container mx-auto px-4 py-40 text-center animate-pulse font-bold text-xl uppercase">Loading...</div>}>
      <ShopContent />
    </Suspense>
  );
}
