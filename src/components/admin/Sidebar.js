...
export default function AdminSidebar() {
  ...
  const [openMenus, setOpenMenus] = useState({
     ...
     users: pathname.includes("/admin/settings/team") || pathname.includes("/admin/settings/roles")
  });
        ...
        <div className="space-y-1 py-2">
           ...
           <div className="group">
                <button 
                  ...
                  className={`w-full flex items-center justify-between px-3 py-2 text-[14px] transition-all ${openMenus.users ? "bg-[#2271b1] text-white" : "hover:bg-[#2c3338] hover:text-[#72aee6]"}`}
                >
                  <div className="flex items-center gap-2">
                      <Shield className="w-4 h-4" />
                      <span>Users & RBAC</span>
                  </div>
                  {openMenus.users ? <ChevronDown className="w-3 h-3" /> : <ChevronRight className="w-3 h-3" />}
                </button>
                {openMenus.users && (
                  <div className="bg-[#2c3338] py-1 border-l-[3px] border-[#2271b1]/30">
                      <NavLink href="/admin/settings/team" exact icon={Users} permission="staff.view">All Staff</NavLink>
                      <NavLink href="/admin/settings/team/new" icon={UserPlus} permission="staff.create">Invite User</NavLink>
