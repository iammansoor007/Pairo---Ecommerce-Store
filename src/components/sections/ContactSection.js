...
export default function ContactSection({
  ...
  const handleSubmit = async (e) => {
    ...
    if (!nameClean || nameClean.length < 2 || nameClean.length > 50 || !/^[A-Za-z\s\-]+$/.test(nameClean)) {
      toast.error("Please enter a valid name (2-50 characters, letters/spaces only).");
      return;
    }
    ...
    if (!messageClean || messageClean.length < 10 || messageClean.length > 1000) {
      toast.error("Message must be between 10 and 1000 characters.");
      return;
    }
    if (formData.hp_field) {
      // Honeypot field filled - silent fail for spam bots
